...
<div class="toolbar">
    <div class="page-title">Manajemen Mapel</div>
    <a href="{{ route('admin.mapel.create') }}" class="btn-primary">+ Tambah Mapel</a>
</div>
...
<div class="table-wrap">
    <table>
        ...
        <tbody>
            @forelse ($mapel as $m)
                <tr>
                    <td class="muted">{{ $loop->iteration + ($mapel->currentPage() - 1) * $mapel->perPage() }}</td>
                    <td>{{ $m->nama_mapel }}</td>
                    <td>{{ $m->kkm }}</td>
                    <td class="aksi">
                        <a class="edit" href="{{ route('admin.mapel.edit', $m) }}">Edit</a>
                        <a class="del" href="{{ route('admin.mapel.show', $m) }}">Detail</a>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="4" class="muted">Belum ada data mapel.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
