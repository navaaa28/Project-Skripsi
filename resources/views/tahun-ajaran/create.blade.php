...
<div class="page-title">Tambah Tahun Ajaran</div>
<div class="card">
    <form method="POST" action="{{ route('admin.tahun-ajaran.store') }}">
        ...
        <div class="field">
            ...
            <select name="nama_tahun_ajaran" class="select" required>
                ...
                @php
                    // Generate opsi dropdown dari 2020/2021 sampai 5 tahun ke depan
                ...
                @foreach ($options as $opt)
                    <option value="{{ $opt }}" @selected(old('nama_tahun_ajaran', $suggested) === $opt)>{{ $opt }}</option>
                @endforeach
            </select>
            <div class="hint">Format otomatis: YYYY/YYYY. Pilih dari daftar, tidak perlu ketik manual.</div>
        </div>
