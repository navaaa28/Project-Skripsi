        ...
        <div class="field">
            ...
            <select name="id_user" class="select" required>
                @foreach ($users as $user)
                    <option value="{{ $user->id_user }}" @selected(old('id_user', $guru->id_user) == $user->id_user)>{{ $user->username }} ({{ $user->email ?? '-' }})</option>
                @endforeach
            </select>
        </div>
        <div class="field">
            <label class="label">NIP</label>
            <input name="nip" value="{{ old('nip', $guru->nip) }}" class="input">
        </div>
        <div class="field">
            <label class="label">Nama Guru</label>
            <input name="nama_guru" value="{{ old('nama_guru', $guru->nama_guru) }}" class="input" required>
        </div>
        <div class="field">
            <label class="label">Jenis Kelamin (L/P)</label>
            <div class="radio-group">
                <label class="radio-item">
                    <input type="radio" name="jenis_kelamin" value="L" @checked(old('jenis_kelamin', $guru->jenis_kelamin) === 'L')>
                    L
                </label>
                <label class="radio-item">
                    <input type="radio" name="jenis_kelamin" value="P" @checked(old('jenis_kelamin', $guru->jenis_kelamin) === 'P')>
                    P
                </label>
            </div>
        </div>
        <div class="field">
            ...
            <select name="mapel_utama" class="select">
                ...
                @foreach ($mapelOptions as $mapel)
                    <option value="{{ $mapel->nama_mapel }}" @selected(old('mapel_utama', $guru->mapel_utama) === $mapel->nama_mapel)>{{ $mapel->nama_mapel }}</option>
                @endforeach
            </select>
        </div>
